...
from .rich_text import parse_rich_text, RichSegment


def _rgba_to_tuple(rgba) -> tuple[int, int, int, int]:
    """将 pyglet 颜色转为 tuple。"""
...
class DialogueSystem:
    ...
    def show_dialogue(self, text: str, speaker: str, entry: dict = None) -> None:
        """显示一句对话。
    ...
    def _process_segment(self, seg: RichSegment) -> None:
        ...
        if seg.type == "text":
            ...
            # 检查 entry-level color 覆盖
            if self._current_color:
                self._text_label.text = self._accumulated_text
                self._text_label.color = self._current_color
            else:
                self._text_label.text = self._accumulated_text
            # 粗体/斜体状态设置
            self._text_label.weight = "bold" if self._bold_active else "normal"
            ...
            self._waiting = True
            self._wait_timer = float(seg.data)
            self._seg_index += 1
            ...
            self._speed_mult = float(seg.data) if seg.data else 1.0
