...
# 正则：匹配 {w=数字} {shake} {/shake} {color=#hex} {/color} {speed=数字} {/speed} {font=名} {/font} {b} {/b} {i} {/i}
    ...
    for m in RICH_PATTERN.finditer(text):
        ...
        if full.startswith("{/"):
            ...
            if tag == "shake":
                ...
                segments.append(RichSegment("endbold"))
            elif tag == "i":
                segments.append(RichSegment("enditalic"))
        else:
            if m.group(1) is not None:  # w=秒数
                ...
                segments.append(RichSegment("bold"))
            elif full == "{i}":
                segments.append(RichSegment("italic"))
